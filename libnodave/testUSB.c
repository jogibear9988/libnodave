...
#ifdef BCCWIN	//makes no sense. Before you can compile, you have to a windows replacement for libUSB
#include <time.h>
    void usage(void);
    void wait(void);
#define WIN_STYLE
...
void usage()
{
    printf("Usage: testUSB [-d] [-w] serial port.\n");
...
#ifdef UNIX_STYLE
    printf("Example: testUSB -w /dev/ttyS0\n");
#endif    
#ifdef WIN_STYLE    
    printf("Example: testUSB -w COM1\n");
#endif    
}
...
int main(int argc, char **argv) {
	...
	doExperimental, doSync, doReadTime, doGetHeaders, doTestMany, aLongDB,
	saveDebug,
    ...
    fds.wfd=fds.rfd;
    initSuccess=0;	
    printf("got USB device handle %p\n",(void *)(fds.rfd));
    if (fds.rfd>0) { 
	di =daveNewInterface(fds, "IF1", localMPI, useProto, speed);
	di->ifread=usbread;
	di->ifwrite=usbwrite;
	daveSetTimeout(di,6000000);
	for (i=0; i<3; i++) {
